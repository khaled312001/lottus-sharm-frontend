      ...
      <Modal onClose={onClose}>
        <div className="p-8 text-center">
          ...
          <h3 className="font-serif text-2xl font-bold text-primary mb-2">
            {L(locale, { ar: 'تم استلام حجزك!', en: 'Booking received!', de: 'Buchung erhalten!', ru: 'Бронь получена!', it: 'Prenotazione ricevuta!' })}
          </h3>
          <p className="text-sm text-muted-foreground leading-relaxed max-w-md mx-auto mb-4">
            {L(locale, {
              ...
              en: 'Our team will review the receipt and confirm within hours. You\'ll get a WhatsApp + email confirmation.', de: "Unser Team prüft den Beleg und bestätigt innerhalb von Stunden. Sie erhalten eine Bestätigung per WhatsApp + E-Mail.",
          ...
          <div className="inline-flex flex-col items-center gap-1 px-5 py-3 rounded-xl bg-accent/10 border border-accent/30">
            <span className="text-[10px] uppercase tracking-wider text-muted-foreground font-bold">
              {L(locale, { ar: 'رقم الحجز', en: 'Booking reference', de: 'Buchungsreferenz', ru: 'Номер брони', it: 'N° prenotazione' })}
            </span>
            <span className="font-mono font-bold text-lg text-accent-700">{success}</span>
          </div>
          <Button onClick={onClose} className="mt-6 w-full">
            {L(locale, { ar: 'إغلاق', en: 'Close', de: 'Schließen', ru: 'Закрыть', it: 'Chiudi' })}
          </Button>
        </div>
      </Modal>
    );
    ...
    <Modal onClose={onClose}>
      <div className="sticky top-0 bg-white border-b px-6 py-4 flex items-center justify-between z-10">
        <div>
          <h3 className="font-serif text-xl font-bold text-primary">
            {L(locale, { ar: 'حجز وادفع', en: 'Book & pay', de: 'Buchen & bezahlen', ru: 'Бронь и оплата', it: 'Prenota e paga' })}
          </h3>
          <p className="text-xs text-muted-foreground mt-0.5">
            {L(locale, {
              ar: 'ارفع إيصال الدفع وفريقنا هيأكد الحجز.',
              en: 'Upload your payment receipt and our team will confirm.', de: 'Laden Sie Ihren Zahlungsbeleg hoch und unser Team bestätigt.',
      ...
      <div className="p-6 space-y-5">
        ...
        <div>
          <label className="block text-xs font-bold text-primary/80 uppercase tracking-wider mb-2">
            {L(locale, { ar: 'اختر طريقة الدفع', en: 'Choose payment method', de: 'Zahlungsmethode wählen', ru: 'Способ оплаты', it: 'Metodo di pagamento' })}
          </label>
          ...
          {current.details.length > 0 ? (
            ...
            <p className="text-xs text-muted-foreground">
              {L(locale, {
                ...
                en: 'Cash on arrival — attach an ID or any reference image to confirm.', de: 'Barzahlung bei Ankunft — fügen Sie zur Bestätigung einen Ausweis oder ein beliebiges Referenzbild bei.',
        ...
        <div>
          <label className="block text-xs font-bold text-primary/80 uppercase tracking-wider mb-2">
            {L(locale, { ar: 'صورة الإيصال', en: 'Payment receipt', de: 'Zahlungsbeleg', ru: 'Чек оплаты', it: 'Ricevuta' })}
          ...
          {!file ? (
            <label className="flex items-center justify-center gap-2 h-14 rounded-xl border-2 border-dashed border-accent/30 hover:border-accent/60 hover:bg-accent/5 cursor-pointer transition-colors text-primary/75">
              ...
              <span className="text-sm font-bold">
                {L(locale, { ar: 'اختر الصورة (أو PDF)', en: 'Choose file (image or PDF)', de: 'Datei wählen (Bild oder PDF)', ru: 'Выбрать файл', it: 'Scegli file' })}
              </span>
            </label>
          ) : (
      ...
      <div className="sticky bottom-0 bg-white border-t px-6 py-4 flex flex-col-reverse sm:flex-row items-stretch sm:items-center justify-end gap-2 z-10">
        <Button variant="outline" onClick={onClose}>
          {L(locale, { ar: 'إلغاء', en: 'Cancel', de: 'Abbrechen', ru: 'Отмена', it: 'Annulla' })}
        </Button>
        <Button onClick={submit} disabled={submitting || (!isCashOnArrival && !file)} className="bg-primary text-cream hover:bg-primary/90">
          ...
          {L(locale, { ar: 'إرسال الحجز للمراجعة', en: 'Submit booking', de: 'Buchung absenden', ru: 'Отправить', it: 'Invia prenotazione' })}
        </Button>
      </div>
    </Modal>
  );
