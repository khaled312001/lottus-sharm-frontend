...
export function AdminShell({ children }: { children: React.ReactNode }) {
  ...
  if (isLogin) return <div dir={dir} lang={locale} className="min-h-screen">{children}</div>;
  if (loading) return <div className="min-h-screen flex items-center justify-center bg-muted/20">{isAr ? 'جاري التحميل...' : 'Loading...'}</div>;
  if (!user) return null;

  // Sidebar lives on the start side: right in RTL, left in LTR.
  // Drawer slides in from the same edge.
        ...
        {drawerOpen && (
          <>
            ...
            <motion.aside
              ...
              className="lg:hidden fixed inset-y-0 start-0 w-72 max-w-[85vw] bg-white shadow-2xl z-50 overflow-y-auto"
            >
