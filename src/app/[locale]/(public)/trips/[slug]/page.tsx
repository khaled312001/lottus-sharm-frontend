            ...
            <div className="mt-6 md:mt-7 flex flex-wrap items-center gap-3 sm:gap-4">
              ...
              {(trip._count?.comments ?? 0) > 0 && (
                <Link href={`/trips/${trip.slug}#comments`} className="inline-flex items-center gap-1.5 px-3 py-2 rounded-full bg-cream/10 backdrop-blur border border-cream/20 text-cream hover:bg-cream/15 transition-colors">
                  ...
                  <span className="text-sm hidden sm:inline">{L(locale, { ar: 'تعليق', en: 'comments', ru: 'комментариев', it: 'commenti' })}</span>
                </Link>
              )}
            </div>
          </Reveal>
          ...
          <Reveal>
            <div>
              <div className="inline-flex items-center gap-2.5 mb-3">
                ...
                <span className="text-accent uppercase tracking-[0.25em] text-[10px] sm:text-[11px] font-bold">{L(locale, { ar: 'عن الرحلة', en: 'About the trip', ru: 'О туре', it: 'Sul tour' })}</span>
              </div>
          ...
          {trip.highlights.length > 0 && (
            <Reveal>
              <div>
                <div className="inline-flex items-center gap-2.5 mb-3">
                  <span className="block w-7 h-px bg-accent" />
                  <span className="text-accent uppercase tracking-[0.25em] text-[10px] sm:text-[11px] font-bold">{L(locale, { ar: 'أبرز المعالم', en: 'Highlights', ru: 'Изюминки', it: 'Punti salienti' })}</span>
                </div>
                <h2 className="font-serif text-2xl md:text-3xl lg:text-4xl font-bold text-primary mb-5 leading-tight text-balance">
          ...
          {(includes.length || excludes.length) > 0 && (
            <Reveal>
              <div>
                <div className="inline-flex items-center gap-2.5 mb-3">
                  <span className="block w-7 h-px bg-accent" />
                  <span className="text-accent uppercase tracking-[0.25em] text-[10px] sm:text-[11px] font-bold">{L(locale, { ar: 'الأسعار شفافة', en: 'Transparent pricing', ru: 'Прозрачные цены', it: 'Prezzi trasparenti' })}</span>
                </div>
                <h2 className="font-serif text-2xl md:text-3xl lg:text-4xl font-bold text-primary mb-5 leading-tight text-balance">
          ...
          {brings.length > 0 && (
            <Reveal>
              <div>
                <div className="inline-flex items-center gap-2.5 mb-3">
                  <span className="block w-7 h-px bg-accent" />
                  <span className="text-accent uppercase tracking-[0.25em] text-[10px] sm:text-[11px] font-bold">{L(locale, { ar: 'استعد بشكل صحيح', en: 'Be prepared', ru: 'Будьте готовы', it: 'Sii preparato' })}</span>
                </div>
            ...
            <div className="relative bg-gradient-to-br from-primary via-primary to-primary-900 text-cream rounded-2xl p-6 md:p-8 overflow-hidden border border-accent/20 shadow-xl">
              ...
              <div className="relative grid md:grid-cols-2 gap-6 md:gap-8 md:divide-x rtl:md:divide-x-reverse md:divide-accent/15">
                <div className="md:pe-8">
                  ...
                  <div className="font-serif text-lg md:text-xl font-bold leading-snug">{(locale === 'ar' && trip.meetingPoint) || L(locale, { ar: trip.meetingPoint || 'فندق الإقامة في شرم الشيخ', en: 'Your Sharm hotel', ru: 'Ваш отель в Шарме', it: 'Il tuo hotel a Sharm' })}</div>
            ...
            <Reveal className="flex items-end justify-between mb-8 md:mb-10 flex-wrap gap-4">
              <div>
                <div className="inline-flex items-center gap-2.5 mb-2.5">
                  ...
                  <span className="text-accent uppercase tracking-[0.3em] text-[10px] sm:text-xs font-bold">{L(locale, { ar: 'رحلات قد تعجبك', en: 'You might also like', ru: 'Также вам понравится', it: 'Potrebbe interessarti anche' })}</span>
                </div>
                <h2 className="font-serif text-2xl md:text-4xl font-bold text-primary leading-tight text-balance">{L(locale, { ar: 'استكشف رحلات أخرى', en: 'Explore other trips', ru: 'Изучите другие туры', it: 'Esplora altri tour' })}</h2>
              </div>
              <Button asChild variant="ghost" className="text-accent-700 hover:text-accent hover:bg-accent/10 group">
                <Link href="/trips">{L(locale, { ar: 'كل الرحلات', en: 'All trips', ru: 'Все туры', it: 'Tutti i tour' })} <ArrowRight className="h-4 w-4 rtl:rotate-180 group-hover:translate-x-1 rtl:group-hover:-translate-x-1 transition-transform" /></Link>
              </Button>
            </Reveal>
