...
function relativeTime(iso: string, locale: string): string {
  ...
  if (diff < day) return L(locale, { ar: 'اليوم', en: 'Today', ru: 'Сегодня', it: 'Oggi' }) as string;
  if (diff < 7 * day) {
    const n = Math.floor(diff / day);
    return L(locale, { ar: `قبل ${n} ${n === 1 ? 'يوم' : 'أيام'}`, en: `${n}d ago`, ru: `${n} дн. назад`, it: `${n}g fa` }) as string;
  }
    ...
    <div>
      <div className="inline-flex items-center gap-2.5 mb-3">
        ...
        <span className="text-accent uppercase tracking-[0.25em] text-[10px] sm:text-[11px] font-bold">
          {L(locale, { ar: 'تقييمات عملائنا', en: 'Customer reviews', ru: 'Отзывы клиентов', it: 'Recensioni clienti' })}
        </span>
      </div>
      <h2 className="font-serif text-2xl md:text-3xl lg:text-4xl font-bold text-primary mb-5 leading-tight text-balance">
        {L(locale, {
          ...
          en: 'Real stories from our travelers',
      ...
      <div className="grid md:grid-cols-[260px_1fr_auto] gap-4 md:gap-6 bg-white rounded-2xl border border-accent/15 p-5 md:p-6 card-shadow">
        ...
        <div className="md:border-e rtl:md:border-e-0 rtl:md:border-s md:border-accent/15 md:pe-6 rtl:md:pe-0 rtl:md:ps-6 flex flex-col items-center md:items-start text-center md:text-start">
          ...
          <div className="text-sm text-muted-foreground mt-2 font-semibold">
            {L(locale, {
              ar: `${total} تقييم موثق`,
              en: `${total} verified review${total === 1 ? '' : 's'}`,
        ...
        <>
          ...
          {items.length > 4 && (
            <div className="mt-5 text-center">
              ...
              >
                {showAll
                  ? (L(locale, { ar: 'عرض أقل', en: 'Show less', ru: 'Свернуть', it: 'Mostra meno' }) as string)
                  : (L(locale, {
                      ar: `عرض كل التقييمات (${items.length})`,
                      en: `View all reviews (${items.length})`,
